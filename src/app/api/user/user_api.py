...
@router.get("/", dependencies=[Depends(JWTBearer())], response_model=List[AllUsersSchema])
async def all_users(db: Session = Depends(get_db)):
    return users(db)


@router.get("/{user_id}", dependencies=[Depends(JWTBearer())], response_model=UserSchema)
async def get_user_by_id(user_id: int, db=Depends(get_db)):
    return user_by_id(db, user_id=user_id)


@router.get("/name/{username}", dependencies=[Depends(JWTBearer())], response_model=UserSchema)
async def get_user_by_username(username: str, db: Session = Depends(get_db)):
    return user_by_username(db, username=username)


@router.get("/email/{email}", dependencies=[Depends(JWTBearer())], response_model=UserSchema)
async def get_user_by_email(email: str, db: Session = Depends(get_db)):
    return user_by_email(db, email=email)


@router.patch("/{user_id}", dependencies=[Depends(JWTBearer())], response_model=UserSchema)
